...
class AppDatabase private constructor(context: Context) : SQLiteOpenHelper(context, "azizi_grocery.db", null, 2) {
    companion object {
    ...
    override fun onUpgrade(db: SQLiteDatabase, oldVersion: Int, newVersion: Int) {
        if (oldVersion < 2) db.execSQL("CREATE TABLE IF NOT EXISTS expenses(id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT, amount REAL, date TEXT, note TEXT)")
    }
    ...
    fun addInvoice(number:Int, customer:String, phone:String, total:Double, payment:String, date:String, time:String, lines:List<SaleLine>):Long {
        ...
        try {
            ...
            val id=db.insertOrThrow("invoices",null,v)
            lines.forEach { l ->
                db.insert("invoice_lines",null,ContentValues().apply { put("invoice_id",id); put("name",l.name); put("qty",l.qty); put("unit",l.unit); put("total",l.total) })
                db.execSQL("INSERT OR IGNORE INTO items(name,price,stock,min_stock) VALUES(?,?,0,0)",arrayOf(l.name,l.unit))
